...
(function($){ 
	...
	$.fn.window=function(options){
		...
		return this.each(function(){
			var _this=this;
			if(options.shadow)
				B = $("<div id='win_overlay' class='dialog-mask'></div>").hide().appendTo('body').show();
			if($(_this).find(".dialog-head").length==0){
				...
				var iframestr="<div class='dialog-head window-header-move' style='width:"+($(_this).width()-2)+"px'><strong class='win_title'>"+title_name+"</strong><span onclick='win_close(this)' class='displog-close close rotate-hover' style='font-size:12px;'>关闭</span></div>";
				var str="<div class='dialog'>"
					+iframestr+"<div class='dialog-body'>"
					+$(_this).html()
					+"</div></div>";
  ...
  $('[class^="icon-"],[class*=" icon-"]').each(function () {
	...
	$('.banner').each(function(){
		...
		if(pointer!=0 && page>1){
			...
			/*var lefter=$('<div class="pager-prev">&nbsp;</div>');
			var righter=$('<div class="pager-next">&nbsp;</div>');
